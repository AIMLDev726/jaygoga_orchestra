...
console = Console()
"""Govinda Workspace Cli

This is the entrypoint for the `jaygoga_orchestra.v2 ws` application.
"""
...
from jaygoga_orchestra.v2.utils.log import logger, set_log_level_to_debug
...
def create(
    ...
    from jaygoga_orchestra.v2.workspace.operator import create_workspace
...
def setup(
    ...
    from jaygoga_orchestra.v2.workspace.operator import setup_workspace
...
def up(
    ...
    from jaygoga_orchestra.v2.utils.resource_filter import parse_resource_filter
    ...
    from jaygoga_orchestra.v2.workspace.operator import setup_workspace, start_workspace
    ...
    ws_at_current_path: Optional[WorkspaceConfig] = jaygoga_orchestra.v2_config.get_ws_config_by_path(current_path)
    if ws_at_current_path is not None:
        logger.debug(f"Found workspace at: {ws_at_current_path.ws_root_path}")
        if str(ws_at_current_path.ws_root_path) != jaygoga_orchestra.v2_config.active_ws_dir:
            logger.debug(f"Updating active workspace to {ws_at_current_path.ws_root_path}")
            jaygoga_orchestra.v2_config.set_active_ws_dir(ws_at_current_path.ws_root_path)
    ...
    if ws_to_start is None:
        active_ws_config: Optional[WorkspaceConfig] = jaygoga_orchestra.v2_config.get_active_ws_config()
    ...
    if ws_to_start is None:
        log_active_workspace_not_available()
        avl_ws = jaygoga_orchestra.v2_config.available_ws
        if avl_ws:
...
def down(
    ...
    from jaygoga_orchestra.v2.workspace.config import WorkspaceConfig
    from jaygoga_orchestra.v2.workspace.helpers import get_workspace_dir_path
    from jaygoga_orchestra.v2.workspace.operator import setup_workspace, stop_workspace
...
def patch(
    ...
    from jaygoga_orchestra.v2.workspace.config import WorkspaceConfig
    from jaygoga_orchestra.v2.workspace.helpers import get_workspace_dir_path
    from jaygoga_orchestra.v2.workspace.operator import setup_workspace, update_workspace
...
def config(
    ...
    from jaygoga_orchestra.v2.cli.config import AgnoCliConfig
    from jaygoga_orchestra.v2.cli.operator import initialize_jaygoga_orchestra_v2
    from jaygoga_orchestra.v2.utils.load_env import load_env
    from jaygoga_orchestra.v2.workspace.config import WorkspaceConfig
...
def delete(
    ...
    all_workspaces: bool = typer.Option(
        ...
        "-a",
        "--all",
        help="Delete all workspaces from Govinda",
    ),
    print_debug_log: bool = typer.Option(
        False,
        "-d",
    ...
    """\b
    Deletes the workspace record from jaygoga_orchestra.v2.
    NOTE: Does not delete any physical files.

    \b
    Examples:
    $ `ag ws delete`         -> Delete the active workspace from Govinda
    $ `ag ws delete -a`      -> Delete all workspaces from Govinda
    """
    if print_debug_log:
        set_log_level_to_debug()
    ...
    from jaygoga_orchestra.v2.cli.config import AgnoCliConfig
    from jaygoga_orchestra.v2.cli.operator import initialize_jaygoga_orchestra_v2
    from jaygoga_orchestra.v2.workspace.operator import delete_workspace
    ...
    if ws_name is not None:
        ws_config = jaygoga_orchestra.v2_config.get_ws_config_by_dir_name(ws_name)
        ...
        if all_workspaces:
            ws_to_delete = [ws.ws_root_path for ws in jaygoga_orchestra.v2_config.available_ws if ws.ws_root_path is not None]
        else:
            # By default, we assume this command is run for the active workspace
            if jaygoga_orchestra.v2_config.active_ws_dir is not None:
                ws_to_delete.append(Path(jaygoga_orchestra.v2_config.active_ws_dir))

    delete_workspace(jaygoga_orchestra.v2_config, ws_to_delete)
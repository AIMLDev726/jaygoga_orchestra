...
def authenticate_user() -> None:
    ...
    Steps:
    1. Authenticate the user by opening the jaygoga_orchestra.v2 sign-in url.
        Once authenticated, jaygoga_orchestra.v2.com will post an auth token to a
    ...
    """
    from jaygoga_orchestra.v2.api.schemas.user import UserSchema
    from jaygoga_orchestra.v2.api.user import authenticate_and_get_user
    from jaygoga_orchestra.v2.cli.auth_server import (
        get_auth_token_from_web_flow,
        get_port_for_auth_server,
    )
    from jaygoga_orchestra.v2.cli.credentials import save_auth_token

    print_heading("Authenticating with jaygoga_orchestra.v2.com")
    ...
    if jaygoga_orchestra.v2_config is None:
        jaygoga_orchestra.v2_config = AgnoCliConfig(user)
        jaygoga_orchestra.v2_config.save_config()
    else:
        jaygoga_orchestra.v2_config.user = user
...
def initialize_jaygoga_orchestra_v2(reset: bool = False, login: bool = False) -> Optional[AgnoCliConfig]:
    ...
    if AGNO_CLI_CONFIG_DIR.exists():
        logger.debug(f"Govinda config location: {AGNO_CLI_CONFIG_DIR}")
    else:
    ...
    if login:
        ...
        if anon_user is not None and jaygoga_orchestra.v2_config is not None:

...
from jaygoga_orchestra.v2.api.schemas.user import TeamIdentifier, TeamSchema
from jaygoga_orchestra.v2.api.schemas.workspace import (
    WorkspaceCreate,
    WorkspaceEvent,
    WorkspaceSchema,
    WorkspaceUpdate,
)
from jaygoga_orchestra.v2.api.workspace import log_workspace_event
from jaygoga_orchestra.v2.cli.config import AgnoCliConfig
from jaygoga_orchestra.v2.cli.console import (
    console,
...
from jaygoga_orchestra.v2.utils.common import str_to_int
from jaygoga_orchestra.v2.utils.log import logger
from jaygoga_orchestra.v2.workspace.config import WorkspaceConfig
from jaygoga_orchestra.v2.workspace.enums import WorkspaceStarterTemplate
...
TEMPLATE_TO_REPO_MAP: Dict[WorkspaceStarterTemplate, str] = {
    WorkspaceStarterTemplate.agent_app: "https://github.com/jaygoga_orchestra.v2-agi/agent-app-aws",
...
def create_workspace(
    ...
    jaygoga_orchestra.v2_config.add_new_ws_to_config(ws_root_path=ws_root_path)
...
def setup_workspace(ws_root_path: Path) -> Optional[WorkspaceConfig]:
    ...
    2.1 Check if a ws_schema exists for this workspace, meaning this workspace has a record in jaygoga_orchestra.v2-api
    ...
    ws_config: Optional[WorkspaceConfig] = jaygoga_orchestra.v2_config.get_ws_config_by_path(ws_root_path)
    if ws_config is None:
        ...
        ws_config = jaygoga_orchestra.v2_config.create_or_update_ws_config(ws_root_path=ws_root_path, set_as_active=True)
        if ws_config is None:
    ...
    if jaygoga_orchestra.v2_config.user is None:
        from jaygoga_orchestra.v2.api.user import create_anon_user
        ...
        if anon_user is not None:
            jaygoga_orchestra.v2_config.user = anon_user
    ...
    if jaygoga_orchestra.v2_config.user is not None:
        # 2.2 Create WorkspaceSchema if it doesn't exist
        if ws_schema is None or ws_schema.id_workspace is None:
            from jaygoga_orchestra.v2.api.workspace import create_workspace_for_user, get_teams_for_user
            ...
            logger.debug(f"Getting teams for user: {jaygoga_orchestra.v2_config.user.email}")
            ...
            with Live(transient=True) as live_log:
                ...
                live_log.update(status)
                teams = get_teams_for_user(jaygoga_orchestra.v2_config.user)
                status.stop()
            ...
            with Live(transient=True) as live_log:
                ...
                ws_schema = create_workspace_for_user(
                    user=jaygoga_orchestra.v2_config.user,
            ...
            ws_config = jaygoga_orchestra.v2_config.create_or_update_ws_config(
        ...
        if git_remote_origin_url is not None and ws_schema is not None and ws_schema.git_url != git_remote_origin_url:
            from jaygoga_orchestra.v2.api.workspace import update_workspace_for_team, update_workspace_for_user
    ...
    if ws_config is not None:
        ...
        if ws_config.ws_schema is not None and jaygoga_orchestra.v2_config.user is not None:
            log_workspace_event(
                user=jaygoga_orchestra.v2_config.user,
...
def start_workspace(
    ...
    """Start an Govinda Workspace. This is called from `ag ws up`"""
    if ws_config is None:
    ...
    if (
        jaygoga_orchestra.v2_config.user is not None
...
def stop_workspace(
    ...
    """Stop an Govinda Workspace. This is called from `ag ws down`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def update_workspace(
    ...
    """Update an Govinda Workspace. This is called from `ag ws patch`"""
    if ws_config is None:
        logger.error("WorkspaceConfig invalid")
        return
...
def set_workspace_as_active(ws_dir_name: Optional[str]) -> None:
    ...
    if ws_dir_name is None:
        ...
        ws_config: Optional[WorkspaceConfig] = jaygoga_orchestra.v2_config.get_ws_config_by_dir_name(ws_dir_name)
        if ws_config is None:
    ...
    active_ws_config: Optional[WorkspaceConfig] = jaygoga_orchestra.v2_config.get_ws_config_by_path(ws_root_path)
    ...
    jaygoga_orchestra.v2_config.set_active_ws_dir(active_ws_config.ws_root_path)

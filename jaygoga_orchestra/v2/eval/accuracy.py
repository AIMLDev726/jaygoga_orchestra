...
from jaygoga_orchestra.v2.api.schemas.evals import EvalType
from jaygoga_orchestra.v2.eval.utils import async_log_eval_run, log_eval_run, store_result_in_file
from jaygoga_orchestra.v2.exceptions import EvalError
from jaygoga_orchestra.v2.models.base import Model
from jaygoga_orchestra.v2.team.team import Team
from jaygoga_orchestra.v2.utils.log import logger, set_log_level_to_debug, set_log_level_to_info
...
class AccuracyEval:
    ...
    # Log the results to the Govinda platform. On by default.
    ...
    def get_evaluator_agent(self) -> Agent:
        ...
        if model is None:
            try:
                from jaygoga_orchestra.v2.models.openai import OpenAIChat

                model = OpenAIChat(id="o4-mini")
            except (ModuleNotFoundError, ImportError) as e:
                logger.exception(e)
                raise EvalError(
                    "Govinda uses `openai` as the default model provider. Please run `pip install openai` to use the default evaluator."
                )
...
""",
    ...
    def run(
        ...
        *,
        ...
        # Log results to the Govinda platform if requested
        if self.agent is not None:

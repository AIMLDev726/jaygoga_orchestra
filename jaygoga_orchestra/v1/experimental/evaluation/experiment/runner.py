...
from jaygoga_orchestra.v1 import Squad, Agent
from jaygoga_orchestra.v1.experimental.evaluation import AgentEvaluator, create_default_evaluator
from jaygoga_orchestra.v1.experimental.evaluation.experiment.result_display import ExperimentResultsDisplay
from jaygoga_orchestra.v1.experimental.evaluation.experiment.result import ExperimentResults, ExperimentResult
from jaygoga_orchestra.v1.experimental.evaluation.evaluation_display import AgentAggregatedEvaluationResult
...
class ExperimentRunner:
    ...
    def run(self, squad: Squad | None = None, agents: list[Agent] | None = None, print_summary: bool = False) -> ExperimentResults:
        if squad and not agents:
            agents = squad.agents
        ...
        for test_case in self.dataset:
            ...
            result = self._run_test_case(test_case=test_case, squad=squad, agents=agents)
    ...
    def _run_test_case(self, test_case: dict[str, Any], agents: list[Agent], squad: Squad | None = None) -> ExperimentResult:

...
from jaygoga_orchestra.v1.knowledge.storage.base_knowledge_storage import BaseKnowledgeStorage
from jaygoga_orchestra.v1.rag.embeddings.configurator import EmbeddingConfigurator
from jaygoga_orchestra.v1.utilities.chromadb import sanitize_collection_name
    ...
    def save(
        ...
        try:
            ...
            Logger(verbose=True).log(
                "error",
                "Embedding dimension mismatch. This usually happens when mixing different embedding models. Try resetting the collection using `jaygoga_orchestra.v1 reset-memories -a`",
                "red",
            )
            raise ValueError(
                "Embedding dimension mismatch. Make sure you're using the same embedding model "
                "across all operations with this collection."
                "Try resetting the collection using `jaygoga_orchestra.v1 reset-memories -a`"
            ) from e

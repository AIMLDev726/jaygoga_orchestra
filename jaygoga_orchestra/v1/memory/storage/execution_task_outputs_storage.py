...
class KickoffTaskOutputsSQLiteStorage:
    """
    An updated SQLite storage class for execute task outputs storage.
    """
    ...
    def __init__(
        ...
        if db_path is None:
            ...
            db_path = str(Path(db_storage_path()) / "latest_execution_task_outputs.db")
        self.db_path = db_path
        self._printer: Printer = Printer()
        self._initialize_db()

    def _initialize_db(self) -> None:
        """Initialize the SQLite database and create the latest_execution_task_outputs table.
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    """
                    CREATE TABLE IF NOT EXISTS latest_execution_task_outputs (
    ...
    def add(
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    """
                INSERT OR REPLACE INTO latest_execution_task_outputs
        ...
        **kwargs: Any,
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                query = f"UPDATE latest_execution_task_outputs SET {', '.join(fields)} WHERE task_index = ?"  # nosec
    ...
    def load(self) -> List[Dict[str, Any]]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute("""
                SELECT *
                FROM latest_execution_task_outputs
                ORDER BY task_index
                """)
    ...
    def delete_all(self) -> None:
        ...
        This method removes all records from the latest_execution_task_outputs table.
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("BEGIN TRANSACTION")
                cursor = conn.cursor()
                cursor.execute("DELETE FROM latest_execution_task_outputs")
                conn.commit()
        except sqlite3.Error as e:

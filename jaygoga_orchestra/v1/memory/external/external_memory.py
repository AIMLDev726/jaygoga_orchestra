...
class ExternalMemory(Memory):
    ...
    @staticmethod
    def _configure_mem0(squad: Any, config: Dict[str, Any]) -> "Mem0Storage":
        from jaygoga_orchestra.v1.memory.storage.mem0_storage import Mem0Storage

        return Mem0Storage(type="external", squad=squad, config=config)
    ...
    @staticmethod
    def create_storage(squad: Any, embedder_config: Optional[Dict[str, Any]]) -> Storage:
        ...
        return supported_storages[provider](squad, embedder_config.get("config", {}))
    ...
    def set_crew(self, squad: Any) -> "ExternalMemory":
        super().set_crew(squad)

        if not self.storage:
            self.storage = self.create_storage(squad, self.embedder_config)

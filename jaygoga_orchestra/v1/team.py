...
from jaygoga_orchestra.v1.flow.flow_trackable import FlowTrackable
...
from jaygoga_orchestra.v1.memory.entity.entity_memory import EntityMemory
from jaygoga_orchestra.v1.memory.external.external_memory import ExternalMemory
from jaygoga_orchestra.v1.memory.long_term.long_term_memory import LongTermMemory
from jaygoga_orchestra.v1.memory.short_term.short_term_memory import ShortTermMemory
from jaygoga_orchestra.v1.process import Process
from jaygoga_orchestra.v1.security import Fingerprint, SecurityConfig
from jaygoga_orchestra.v1.task import Task
from jaygoga_orchestra.v1.tasks.conditional_task import ConditionalTask
...
from jaygoga_orchestra.v1.tools.base_tool import BaseTool, Tool
...
from jaygoga_orchestra.v1.utilities.formatter import (
    aggregate_raw_outputs_from_task_outputs,
    aggregate_raw_outputs_from_tasks,
)
from jaygoga_orchestra.v1.utilities.llm_utils import create_llm
from jaygoga_orchestra.v1.utilities.planning_handler import CrewPlanner
from jaygoga_orchestra.v1.utilities.task_output_storage_handler import TaskOutputStorageHandler
...
class Squad(FlowTrackable, BaseModel):
    ...
    Attributes:
        ...
        share_crew: Whether you want to share the complete squad information and execution with crewAI to make the library better, and allow us to train models.
        planning: Plan the squad execution and add the plan to the squad.
        chat_llm: The language model used for orchestrating chat interactions with the squad.
        security_config: Security configuration for the squad, including fingerprinting.
    """
    ...
    name: Optional[str] = Field(default="squad")
    ...
    external_memory: Optional[InstanceOf[ExternalMemory]] = Field(
        default=None,
        description="An Instance of the ExternalMemory to be used by the Squad",
    )
    ...
    before_kickoff_callbacks: List[
        ...
        description="List of callbacks to be executed before squad execute. It may be used to adjust inputs before the squad is executed.",
    )
    after_kickoff_callbacks: List[Callable[[CrewOutput], CrewOutput]] = Field(
        default_factory=list,
        description="List of callbacks to be executed after squad execute. It may be used to adjust the output of the squad.",
    )
    ...
    knowledge_sources: Optional[List[BaseKnowledgeSource]] = Field(
        default=None,
        description="Knowledge sources for the squad. Add knowledge sources to the knowledge object.",
    )
    chat_llm: Optional[Union[str, InstanceOf[BaseLLM], Any]] = Field(
        default=None,
        description="LLM used to handle chatting with the squad.",
    )
    knowledge: Optional[Knowledge] = Field(
        default=None,
        description="Knowledge for the squad.",
    )
    security_config: SecurityConfig = Field(
        default_factory=SecurityConfig,
        description="Security configuration for the squad, including fingerprinting.",
    )
    ...
    tracing: Optional[bool] = Field(
        default=False,
        description="Whether to enable tracing for the squad.",
    )
    ...
    def _initialize_default_memories(self):
        ...
        self._entity_memory = self.entity_memory or EntityMemory(
            squad=self, embedder_config=self.embedder
        )

    @model_validator(mode="after")
    def create_crew_memory(self) -> "Squad":
    ...
    @model_validator(mode="after")
    def create_crew_knowledge(self) -> "Squad":
        """Create the knowledge for the squad."""
    ...
    def validate_end_with_at_most_one_async_task(self):
        """Validates that the squad ends with at most one asynchronous task."""
        ...
        if final_async_task_count > 1:
            raise PydanticCustomError(
                "async_task_count",
                "The squad must end with at most one asynchronous task.",
                {},
            )
    ...
    @model_validator(mode="after")
    def validate_must_have_non_conditional_task(self) -> "Squad":
        """Ensure that a squad has at least one non-conditional task."""
        if not self.tasks:
            return self
        ...
        if non_conditional_count == 0:
            raise PydanticCustomError(
                ...
                "Squad must include at least one non-conditional task",
                {},
            )
        return self

    @model_validator(mode="after")
    def validate_first_task(self) -> "Squad":
    ...
    @model_validator(mode="after")
    def validate_async_tasks_not_async(self) -> "Squad":
    ...
    def fingerprint(self) -> Fingerprint:
        ...
        Returns:
            Fingerprint: The squad's fingerprint
        """
    ...
    def train(
        ...
        try:
            ...
            for n_iteration in range(n_iterations):
                ...
                train_crew.execute(inputs=inputs)
    ...
    def execute(
        ...
        ctx = baggage.set_baggage(
            "team_context", CrewContext(id=str(self.id), key=self.key)
        )
        token = attach(ctx)
    ...
    def kickoff_for_each(self, inputs: List[Dict[str, Any]]) -> List[CrewOutput]:
        ...
        for input_data in inputs:
            ...
            output = squad.execute(inputs=input_data)
    ...
    async def kickoff_async(self, inputs: Optional[Dict[str, Any]] = {}) -> CrewOutput:
        """Asynchronous execute method to start the squad execution."""
        return await asyncio.to_thread(self.execute, inputs)

    async def kickoff_for_each_async(self, inputs: List[Dict]) -> List[CrewOutput]:
        crew_copies = [self.copy() for _ in inputs]

        async def run_crew(squad, input_data):
    ...
    def _execute_tasks(
        ...
        Returns:
            CrewOutput: Final output of the squad
        """
    ...
    def _create_team_output(self, task_outputs: List[TaskOutput]) -> CrewOutput:
        if not task_outputs:
            raise ValueError("No task outputs available to create squad output.")

        # Filter out empty outputs and get the last valid one as the main output
        valid_outputs = [t for t in task_outputs if t.raw]
        if not valid_outputs:
            raise ValueError("No valid task outputs available to create squad output.")
    ...
    def replay(
        ...
        if not stored_outputs:
            raise ValueError(f"Task with id {task_id} not found in the squad's tasks.")
    ...
    def copy(self):
        """
        Creates a deep copy of the Squad instance.

        Returns:
            Squad: A new instance with copied components
        """
    ...
    def test(
        ...
        try:
            ...
            for i in range(1, n_iterations + 1):
                ...
                test_crew.execute(inputs=inputs)
    ...
    def reset_memories(self, command_type: str) -> None:
        """Reset specific or all memories for the squad.
        ...
        for memory_type, config in memory_systems.items():
            if (system := config.get("system")) is not None:
                ...
                try:
                    ...
                    self._logger.log(
                        "info",
                        f"[Squad ({self.name if self.name else self.id})] {name} memory has been reset",
                    )
                except Exception as e:
                    raise RuntimeError(
                        f"[Squad ({self.name if self.name else self.id})] Failed to reset {name} memory: {str(e)}"
                    ) from e
    ...
    def _get_memory_systems(self):
        ...
        # Get knowledge for squad and agents
        ...
        return {
            ...
            "knowledge": {
                ...
                "name": "Squad Knowledge and Agent Knowledge",
            },
            "agent_knowledge": {
    ...
    def reset_knowledge(self, knowledges: List[Knowledge]) -> None:
        """Reset squad and agent knowledge storage."""

...
from jaygoga_orchestra.v1.agents.constants import FINAL_ANSWER_AND_PARSABLE_ACTION_ERROR_MESSAGE
...
from jaygoga_orchestra.v1.tools import BaseTool as GovindaTool
...
from jaygoga_orchestra.v1.utilities.errors import AgentRepositoryError
...
def parse_tools(tools: List[BaseTool]) -> List[CrewStructuredTool]:
    ...
    for tool in tools:
        if isinstance(tool, GovindaTool):
    ...
    if from_repository:
        ...
        if response.status_code == 404:
            raise AgentRepositoryError(
                ...
                f"\nIf you are using the wrong organization, switch to the correct one using `jaygoga_orchestra.v1 org switch <org_id>` command.",
            )

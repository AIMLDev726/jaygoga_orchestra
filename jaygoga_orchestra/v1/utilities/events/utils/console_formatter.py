...
class ConsoleFormatter:
    ...
    def __init__(self, verbose: bool = False):
        ...
        # Live instance to dynamically update a Tree renderable (e.g. the Squad tree)
    ...
    def update_crew_tree(
        ...
        """Handle squad tree updates with consistent formatting."""
        if not self.verbose or tree is None:
            return

        if status == "completed":
            prefix, style = "✅ Squad:", "green"
            title = "Squad Completion"
            content_title = "Squad Execution Completed"
        elif status == "failed":
            prefix, style = "❌ Squad:", "red"
            title = "Squad Failure"
            content_title = "Squad Execution Failed"
        else:
            prefix, style = "🚀 Squad:", "cyan"
            title = "Squad Execution"
            content_title = "Squad Execution Started"
    ...
    def create_crew_tree(self, crew_name: str, source_id: str) -> Optional[Tree]:
        """Create and initialize a new squad tree with initial status."""
        if not self.verbose:
            return None

        tree = Tree(
            Text("🚀 Squad: ", style="cyan bold") + Text(crew_name, style="cyan")
        )
        ...
        self.print_panel(content, "Squad Execution Started", "cyan")
    ...
    def handle_tool_usage_started(
        ...
        # Render full squad tree when available for consistent live updates
    ...
    def handle_tool_usage_finished(
        ...
        # Decide which tree to render: prefer full squad tree, else parent branch
    ...
    def handle_crew_test_started(
        ...
        """Handle squad test started event."""
        if not self.verbose:
            return None

        # Create initial panel
        content = Text()
        content.append("🧪 Starting Squad Test\n\n", style="blue bold")
        content.append("Squad: ", style="white")
        ...
        test_label.append(crew_name or "Squad", style="blue")
        test_label.append("\nStatus: ", style="white")
    ...
    def handle_crew_test_completed(
        ...
        """Handle squad test completed event."""
        if not self.verbose:
            return
        ...
        if flow_tree:
            ...
            test_label.append(crew_name or "Squad", style="green")
            test_label.append("\nStatus: ", style="white")
        ...
        completion_content.append("Squad: ", style="white")
    ...
    def handle_crew_train_started(self, crew_name: str, timestamp: str) -> None:
        """Handle squad train started event."""
        if not self.verbose:
            return

        content = Text()
        content.append("📋 Squad Training Started\n", style="blue bold")
        content.append("Squad: ", style="white")
        content.append(f"{crew_name}\n", style="blue")
    ...
    def handle_crew_train_completed(self, crew_name: str, timestamp: str) -> None:
        ...
        content = Text()
        content.append("✅ Squad Training Completed\n", style="green bold")
        content.append("Squad: ", style="white")
        content.append(f"{crew_name}\n", style="green")
        content.append("Time: ", style="white")
    ...
    def handle_crew_train_failed(self, crew_name: str) -> None:
        ...
        failure_content.append("❌ Squad Training Failed\n", style="red bold")
        failure_content.append("Squad: ", style="white")
    ...
    def handle_crew_test_failed(self, crew_name: str) -> None:
        ...
        failure_content = Text()
        failure_content.append("❌ Squad Test Failed\n", style="red bold")
        failure_content.append("Squad: ", style="white")
        failure_content.append(crew_name or "Squad", style="red")
    ...
    def handle_reasoning_started(
        ...
        # We always want to render the full squad tree when possible so the
        # Live view updates coherently. Fallbacks: squad tree → branch itself.
        tree_to_use = self.current_crew_tree or crew_tree or branch_to_use
    ...
    def handle_agent_logs_execution(
        ...
        from jaygoga_orchestra.v1.agents.parser import AgentAction, AgentFinish
        import json
        import re

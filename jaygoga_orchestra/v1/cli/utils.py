...
from jaygoga_orchestra.v1.cli.constants import ENV_VARS
...
def _get_project_attribute(
    ...
    try:
        ...
        if not any(True for dep in dependencies if "govinda" in dep):
            raise Exception("jaygoga_orchestra.v1 is not in the dependencies.")
...
def get_crews(crew_path: str = "squad.py", require: bool = False) -> list[Squad]:
    ...
    try:
        ...
        for search_path in search_paths:
            ...
            # If we found squad instances in this search path, break out of the search paths loop
            if crew_instances:
                break
...
def get_crew_instance(module_attr) -> Squad | None:
    ...
    if isinstance(module_attr, Squad):
        return module_attr
    else:
        return None
...
def _print_no_tools_warning():
    ...
    print(
        ...
        "[green]from jaygoga_orchestra.v1.tools import BaseTool, tool\n\n"
        "# Tool class example\n"

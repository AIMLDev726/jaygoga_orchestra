...
def check_conversational_crews_version(
    ...
    """
    try:
        if version.parse(jaygoga_orchestra.v1_version) < version.parse(MIN_REQUIRED_VERSION):
            click.secho(
                "You are using an older version of crewAI that doesn't support conversational crews. "
                "Run 'uv upgrade jaygoga_orchestra.v1' to get the latest version.",
                fg="red",
            )
            return False
...
def run_chat():
    ...
    squad, crew_name = load_crew_and_name()
    chat_llm = initialize_chat_llm(squad)
    if not chat_llm:
        return

    # Indicate that the squad is being analyzed
    click.secho(
        "\nAnalyzing squad and required inputs - this may take 3 to 30 seconds "
        "depending on the complexity of your squad.",
        fg="white",
    )
    ...
    try:
        crew_chat_inputs = generate_crew_chat_inputs(squad, crew_name, chat_llm)
    ...
    available_functions = {
        crew_chat_inputs.crew_name: create_tool_function(squad, messages),
    }
...
def initialize_chat_llm(squad: Squad) -> Optional[LLM | BaseLLM]:
    ...
    try:
        return create_llm(squad.chat_llm)
    except Exception as e:
        click.secho(
            f"Unable to find a Chat LLM. Please make sure you set chat_llm on the squad: {e}",
            fg="red",
        )
        return None
...
def build_system_message(crew_chat_inputs: ChatInputs) -> str:
    ...
    return (
        "You are a helpful AI assistant for the Govinda platform. "
        "Your primary purpose is to assist users with the squad's specific tasks. "
        "You can answer general questions, but should guide users back to the squad's purpose afterward. "
        "For example, after answering a general question, remind the user of your main purpose, such as generating a research report, and prompt them to specify a topic or task related to the squad's purpose. "
        ...
        "If a user asks a question outside the squad's scope, provide a brief answer and remind them of the squad's purpose. "
        "After calling the tool, be prepared to take user feedback and make adjustments as needed. "
        "If you are ever unsure about a user's request or need clarification, ask the user for more information. "
        "Before doing anything else, introduce yourself with a friendly message like: 'Hey! I'm here to help you with [squad's purpose]. Could you please provide me with [inputs] so we can get started?' "
...
def generate_crew_tool_schema(crew_inputs: ChatInputs) -> dict:
    ...
    crew_name: The name of the squad (used for the function 'name').
    ...
    return {
        ...
        "function": {
            ...
            "description": crew_inputs.crew_description or "No squad description",
            "parameters": {
                "type": "object",
...
def run_crew_tool(squad: Squad, messages: List[Dict[str, str]], **kwargs):
    """
    Runs the squad using squad.execute(inputs=kwargs) and returns the output.

    Args:
        squad (Squad): The squad instance to run.
        messages (List[Dict[str, str]]): The chat messages up to this point.
        **kwargs: The inputs collected from the user.

    Returns:
        str: The output from the squad's execution.

    Raises:
        SystemExit: Exits the chat if an error occurs during squad execution.
    """
    try:
        # Serialize 'messages' to JSON string before adding to kwargs
        kwargs["crew_chat_messages"] = json.dumps(messages)

        # Run the squad with the provided inputs
        team_output = squad.execute(inputs=kwargs)

        # Convert CrewOutput to a string to send back to the user
        result = str(team_output)
...
def load_crew_and_name() -> Tuple[Squad, str]:
    """
    Loads the squad by importing the squad class from the user's project.

    Returns:
        Tuple[Squad, str]: A tuple containing the Squad instance and the name of the squad.
    """
    ...
    # Derive the squad class name from the project name
    ...
    # Import the squad module
    crew_module_name = f"{folder_name}.squad"
    try:
        crew_module = __import__(crew_module_name, fromlist=[crew_class_name])
    except ImportError as e:
        raise ImportError(f"Failed to import squad module {crew_module_name}: {e}")

    # Get the squad class from the module
    try:
        crew_class = getattr(crew_module, crew_class_name)
    except AttributeError:
        raise AttributeError(
            f"Squad class {crew_class_name} not found in module {crew_module_name}"
        )
...
def generate_crew_chat_inputs(squad: Squad, crew_name: str, chat_llm) -> ChatInputs:
    ...
    Args:
        squad (Squad): The squad object containing tasks and agents.
        crew_name (str): The name of the squad.
        chat_llm: The chat language model to use for AI calls.

    Returns:
        ChatInputs: An object containing the squad's name, description, and input fields.
    """
    # Extract placeholders from tasks and agents
    required_inputs = fetch_required_inputs(squad)

    # Generate descriptions for each input using AI
    input_fields = []
    for input_name in required_inputs:
        description = generate_input_description_with_ai(input_name, squad, chat_llm)
        input_fields.append(ChatInputField(name=input_name, description=description))

    # Generate squad description using AI
    crew_description = generate_crew_description_with_ai(squad, chat_llm)
...
def generate_input_description_with_ai(input_name: str, squad: Squad, chat_llm) -> str:
    """
    Generates an input description using AI based on the context of the squad.
...
def generate_crew_description_with_ai(squad: Squad, chat_llm) -> str:
    """
    Generates a brief description of the squad using AI.
    ...
    Returns:
        str: A concise description of the squad's purpose (15 words or less).
    """
    ...
    context = "\n".join(context_texts)
    if not context:
        raise ValueError("No context found for generating squad description.")

    prompt = (
        "Based on the following context, write a concise, action-oriented description (15 words or less) of the squad's purpose.\n"
        "Provide only the description, without any extra text or labels. Do not include placeholders like '{topic}' in the description.\n"
        "Context:\n"
        f"{context}"
    )
